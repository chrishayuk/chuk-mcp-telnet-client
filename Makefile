...
# Run the MCP server
run:
	@echo "Running Telnet Client MCP server..."
	@if command -v uv >/dev/null 2>&1; then \
		PYTHONPATH=src uv run python -m chuk_mcp_telnet_client.main; \
	else \
		PYTHONPATH=src python3 -m chuk_mcp_telnet_client.main; \
	fi
...
docker-build:
	...
	docker build -t chuk-mcp-telnet-client:latest .
	@echo "Docker image built: chuk-mcp-telnet-client:latest"

# Run Docker container
docker-run:
	@echo "Running Docker container..."
	docker run --rm -p 8000:8000 chuk-mcp-telnet-client:latest
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-telnet-client==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-telnet-client==$$version"
